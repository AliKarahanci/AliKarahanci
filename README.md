## Hi there

\[JP]: 「4年間の学業を通じ、オブジェクト指向プログラミングからフルスケールのアプリケーション開発に至るまで、多岐にわたる専門技術を習得してまいりました。私は単に技術を享受する側ではなく、自ら技術を創造する側であることを重視しております。そのため、オープンソースプロジェクトへの参画や支援にも積極的に取り組んでおります。」

\[En]: During my four-year academic journey, I have gained a wide range of competencies, from Object-Oriented Programming to full-scale application development. I prioritize being on the side that builds technology rather than just consuming it; therefore, I actively support and contribute to open-source projects.

![Ali's GitHub stats](https://github-readme-stats.vercel.app/api?username=AliKarahanci)

[![My Skills](https://skillicons.dev/icons?i=bun,c,cs,cpp,rust,python,ubuntu,debian,redhat,linux,bsd,androidstudio)](https://skillicons.dev)

### Projects && Guides
* [`Linux Rehberi (Turkish)`](https://github.com/AliKarahanci/linux-rehberi) - Türkçe / Linux kullanım ve tercih rehberi
* [`wp-linux`](https://github.com/AliKarahanci/wp-linux) - Whatsapp client for Linux.

## Social
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/karahanciali) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:karahanciali@hotmail.com) 
